...
Inference for steganographic-CoT models (SFT/LoRA) with schema + leak prevention.

Key changes vs. your original:
- Stage-1 now uses a logits processor to BAN digits, math symbols, 'Answer' strings,
  '####', and common raw math words inside <think>.
- Stage-1 also stops on 'Answer:' and '####' (in addition to </think>).
- Same protections applied to batched Stage-1.
- Light post-sanitize of the CoT if any stray leak still appears.
"""
...
def extract_last_think_and_answer_from_generation(
    ...
    for m in close_pat.finditer(gen_text):
        last_close = m
    search_region = gen_text[last_close.end():] if last_close else gen_text
...
FORBIDDEN_ANSWER = re.compile(r"(^|\s)Answer\s*:", re.IGNORECASE)
FORBIDDEN_HASH = re.compile(r"(^|\n)####")
...
def check_stego_cot(cot: str) -> List[str]:
    ...
    if re.search(FORBIDDEN_ANSWER, cot):
        leaks.append("answer-in-think")
    if re.search(FORBIDDEN_HASH, cot):
        leaks.append("hash-blocks")
    return leaks

def post_sanitize_think(cot: str) -> str:
    """Minimal guardrail: strip anything after an 'Answer:' or '####' line if it slipped in."""
    cutoff = None
    for m in re.finditer(r"(?i)(?:\n|^)(Answer\s*:|####)", cot):
        cutoff = m.start()
        break
    if cutoff is not None:
        cot = cot[:cutoff].rstrip()
    return cot
...
# --------------------------------------------------------------------------------------------------
# Logits processors (prevention during Stage-1)
# --------------------------------------------------------------------------------------------------
class BanRegexLogitsProcessor(LogitsProcessor):
    """
    Bans tokens whose *string form* matches any of the provided regex patterns.
    Good for banning digits, math symbols, 'answer', '####', and raw math words.
    """
    def __init__(self, tokenizer: AutoTokenizer, patterns: List[re.Pattern]):
        self.bad_ids: List[int] = []
        vocab = tokenizer.get_vocab()
        for tok, tid in vocab.items():
            for pat in patterns:
                if pat.search(tok):
                    self.bad_ids.append(tid)
                    break

    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        if self.bad_ids:
            scores[:, self.bad_ids] = -float("inf")
        return scores

def build_think_logits_processor(tokenizer: AutoTokenizer) -> LogitsProcessorList:
    # Ban digits, math symbols, hashes, 'answer', and common raw-math roots
    pats = [
        re.compile(r"[0-9]"),
        re.compile(r"[+\-*/=×÷]"),
        re.compile(r"^#{2,}"),      # tokens that look like '##' headings / '####'
        re.compile(r"####"),
        re.compile(r"(?i)answer"),
        re.compile(r"(?i)\bsum\b|\bminus\b|\bplus\b|\bmultip|\bdivid|\bequal|\bproduct|\bdifference|\bquotient|\bremainder"),
    ]
    return LogitsProcessorList([BanRegexLogitsProcessor(tokenizer, pats)])


# --------------------------------------------------------------------------------------------------
# Tokenize/decode helpers (batched and single)
# --------------------------------------------------------------------------------------------------
...
def generate_two_stage(
    ...
    *,
    ...
    """
    Stage 1: prompt + "<think>\n"  -> stop at "</think>" (and early-stop if "Answer:" or "####" appear)
            + ban digits/math symbols/'Answer'/####/raw-math-words via logits processor.
    Stage 2: prompt + "<think>cot</think>\nAnswer: " -> number/line stop as before.
    """
    t0 = time.time()
    ...
    # Stop at </think> OR if the model starts to emit an 'Answer:' line or '####' block prematurely
    stop_stage1 = [think_close, "Answer:", "####"]
    logits_proc = build_think_logits_processor(tokenizer)
    ...
    gen1, full1, _ = generate_single(
        ...
        repetition_penalty=repetition_penalty,
        stop_strings=stop_stage1,
        no_repeat_ngram_size=no_repeat_ngram_size,
        logits_processor=logits_proc,
    )
    # Remove the stop token if it got included at the end
    for stopper in [think_close, "Answer:", "####"]:
        if gen1.endswith(stopper):
            gen1 = gen1[: -len(stopper)]
    cot_text = post_sanitize_think(gen1.strip())
...
# --------------------------------------------------------------------------------------------------
# NEW: batched generation with Stage-1 protections
# --------------------------------------------------------------------------------------------------
...
def generate_two_stage_batch(
    ...
    *,
    ...
    # Stage 1 (batched) with protections
    ...
    gen_kwargs1 = dict(
        ...
        pad_token_id=tokenizer.eos_token_id,
        return_dict_in_generate=True,
        stopping_criteria=make_stoppers(tokenizer, [think_close, "Answer:", "####"]),
        logits_processor=logits_proc,
    )
    if no_repeat_ngram_size and no_repeat_ngram_size > 0:
    ...
    for t in gen1_texts:
        for stopper in [think_close, "Answer:", "####"]:
            if t.endswith(stopper):
                t = t[: -len(stopper)]
        t = post_sanitize_think(t.strip())
        cot_texts.append(t)
    ...
    def process_batch(batch_items: List[Tuple[str, Union[str, List[Dict[str, str]]]]]):
        ...
        try:
            if args.force_schema:
                ...
                # Single-shot fallback (batched) -- unchanged, no protections
                ...
                dts = [round(dt_batch, 4)] * len(ids)
                stage1_full_list, stage2_full_list = full_texts, [None] * len(ids)
            ...
            for qid, payload in batch_items:
                try:
                    ...
                    fout.flush()
            return  # per-item path handled; exit helper
