...
echo "$(date) - starting polling_loop_thirdwait.sh"
...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/polling_loop_thirdwait-$(date '+%Y%m%d_%H%M%S').log"

if [[ -z "${DETACHED_polling_loop_thirdwait:-}" ]]; then
  export DETACHED_polling_loop_thirdwait=1      # mark the second copy
  exec nohup caffeinate -dimsu "$0" -k "$GOOGLE_API_KEY" \
       "$FIRST_TYPE" "$SECOND_TYPE" "$THIRD_TYPE" "$FOURTH_TYPE" \
       </dev/null >>"$LOG_FILE" 2>&1 &
  disown
  echo "polling_loop_thirdwait detached → log → $LOG_FILE"
  exit 0
fi
...
pid=$(pgrep -f "assess_loop_mmlu_q.sh $FIRST_TYPE 1 2 3 4" | head -n1)
if [[ -z "$pid" ]]; then
