...
# chmod +x c_assess_inf/hpc/results_assess_2.sh
#
#   # run all 11 types
#   caffeinate -dimsu ./c_assess_inf/hpc/results_assess_2.sh
#
#   # run only specific types
#   caffeinate -dimsu ./c_assess_inf/hpc/results_assess_2.sh context obstruction syntax voice
...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/coobsyvo_batch-$(date '+%Y%m%d_%H%M%S').txt"
...
for TYPE in "${TYPES[@]}"; do
  ...
  LOG_FILE="$LOG_DIR/coobsyvo_${TYPE}-${TS}.txt"
