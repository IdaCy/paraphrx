...
echo "$(date) - starting polling_loop.sh"
...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/polling_loop-$(date '+%Y%m%d_%H%M%S').log"

if [[ -z "${DETACHED_polling_loop:-}" ]]; then
  export DETACHED_polling_loop=1      # mark the second copy
  exec nohup caffeinate -dimsu "$0" -k "$GOOGLE_API_KEY" \
       "$FIRST_TYPE" "$SECOND_TYPE" \
       </dev/null >>"$LOG_FILE" 2>&1 &
  disown
  echo "polling_loop detached → log → $LOG_FILE"
  exit 0
fi
...
pid=$(pgrep -f "c_assess_inf/hpc/assess_loop_mmlu.sh $FIRST_TYPE 1 2 3 4 5" | head -n1)
if [[ -z "$pid" ]]; then
