...
# chmod +x c_assess_inf/hpc/results_q_assess_3.sh
#
...
#
#   # run all 11 types
#   caffeinate -dimsu ./c_assess_inf/hpc/results_q_assess_3.sh
#
#   # run only specific types
#   caffeinate -dimsu ./c_assess_inf/hpc/results_q_assess_3.sh language
...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/q3_batch-$(date '+%Y%m%d_%H%M%S').txt"
...
for TYPE in "${TYPES[@]}"; do
  ...
  LOG_FILE="$LOG_DIR/q3__${TYPE}-${TS}.txt"
