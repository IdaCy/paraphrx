...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/a_9_batch-$(date '+%Y%m%d_%H%M%S').txt"
exec >>"$MASTER_LOG" 2>&1
set -x

TS_START=$(date '+%Y-%m-%d %H:%M:%S')
echo "$TS_START – a g9 batch run started" >>"$HOME/times.log"
...
for SLICE in "${SLICES[@]}"; do
  ...
  LOG_FILE="$LOG_DIR/alpaca_g9__${SLICE_TAG}-${TS}.txt"
...
echo "$TS_END – a g9 batch run finished" >>"$HOME/times.log"
exit 0
