...
echo "$(date) - starting polling_loop_voice12345.sh"
...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/polling_loop_voice12345-$(date '+%Y%m%d_%H%M%S').log"

if [[ -z "${DETACHED_polling_loop_voice12345:-}" ]]; then
  export DETACHED_polling_loop_voice12345=1      # mark the second copy
  exec nohup caffeinate -dimsu "$0" -k "$GOOGLE_API_KEY" \
       "$FIRST_TYPE" "$SECOND_TYPE" \
       </dev/null >>"$LOG_FILE" 2>&1 &
  disown
  echo "polling_loop_voice12345 detached → log → $LOG_FILE"
  exit 0
fi
...
echo "$(date) – started follow-up batch for $SECOND_TYPE 1 2 3 4 5"
