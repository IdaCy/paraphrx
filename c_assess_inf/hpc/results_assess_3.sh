...
# chmod +x c_assess_inf/hpc/results_assess_3.sh
#
#   # run all 11 types
#   caffeinate -dimsu ./c_assess_inf/hpc/results_assess_3.sh
#
#   # run only specific types
#   caffeinate -dimsu ./c_assess_inf/hpc/results_assess_3.sh extra length speci_char
...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/exlesp_batch-$(date '+%Y%m%d_%H%M%S').txt"
...
for TYPE in "${TYPES[@]}"; do
  ...
  LOG_FILE="$LOG_DIR/exlesp_${TYPE}-${TS}.txt"
