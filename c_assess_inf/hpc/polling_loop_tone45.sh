...
echo "$(date) - starting polling_loop_tone45.sh"
...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/polling_loop_tone45-$(date '+%Y%m%d_%H%M%S').log"

if [[ -z "${DETACHED_polling_loop_tone45:-}" ]]; then
  export DETACHED_polling_loop_tone45=1      # mark the second copy
  exec nohup caffeinate -dimsu "$0" -k "$GOOGLE_API_KEY" \
       "$FIRST_TYPE" "$SECOND_TYPE" \
       </dev/null >>"$LOG_FILE" 2>&1 &
  disown
  echo "polling_loop_tone45 detached → log → $LOG_FILE"
  exit 0
fi
...
pid=$(pgrep -f "assess_loop_mmlu_q.sh $FIRST_TYPE 4 5" | head -n1)
if [[ -z "$pid" ]]; then
...
caffeinate -dimsu ./c_assess_inf/hpc/assess_loop_mmlu_q.sh \
           -k "$GOOGLE_API_KEY"  "$SECOND_TYPE" 4 5

echo "$(date) – started follow-up batch for $SECOND_TYPE 4 5"
