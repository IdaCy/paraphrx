...
# Usage:
# chmod +x ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice_short.sh
# nohup ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice_short.sh >  logs/prx_modelchoice_short_$(date +%Y%m%d_%H%M%S).log 2>&1 & disown
#
# ps -f -u "$USER" | grep run_prx_alpaca_local_modelchoice_short.sh | grep -v grep
