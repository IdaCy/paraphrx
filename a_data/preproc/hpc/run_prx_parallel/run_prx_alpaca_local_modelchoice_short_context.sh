...
# Usage:
# chmod +x ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice_short_context.sh
# nohup ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice_short_context.sh >  logs/prx_modelchoice_short_context_$(date +%Y%m%d_%H%M%S).log 2>&1 & disown
#
# ps -f -u "$USER" | grep run_prx_alpaca_local_modelchoice_short_context.sh | grep -v grep
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - short_context_2_prx_alpaca_gen_phrx started" >> "$WORKDIR/times.log"
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - short_context_2_prx_alpaca_gen_phrx finished" >> "$WORKDIR/times.log"
