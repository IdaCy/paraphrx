...
# Usage:
# chmod +x ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice.sh
# nohup ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice.sh >  logs/prx_modelchoice_$(date +%Y%m%d_%H%M%S).log 2>&1 & disown
#
# ps -f -u "$USER" | grep run_prx_alpaca_local_modelchoice.sh | grep -v grep
#all? ->
# ps -f -ww -u "$USER" | grep -Ei 'run_prx_alpaca_local_modelchoice' | grep -v grep
# every wrapper script still running:
# ps -f -ww -u "$USER" | grep -E 'run_prx_alpaca_local_modelchoice.*\.sh' | grep -v grep
...
for SLICE in 2 3 4 5; do
  IN_JSON="$DATA_DIR/alpaca/un_prxed/slice${SLICE}.json"

  for TYPE in extra boundary length obstruction special_chars style syntax tone voice; do
