...
# Usage:
# chmod +x ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice_onextra.sh
# nohup ./a_data/preproc/hpc/run_prx_alpaca_local_modelchoice_onextra.sh >  logs/prx_modelchoice_$(date +%Y%m%d_%H%M%S).log 2>&1 & disown
#
# ps -f -u "$USER" | grep run_prx_alpaca_local_modelchoice_onextra.sh | grep -v grep
