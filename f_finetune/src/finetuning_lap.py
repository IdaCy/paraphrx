...
Original LoRA-NF4 4-bit adapters:
...
def setup_logging(run_name: str, output_dir: str):
    ...
    # Define a custom format with timestamps
        ...
        for item in data:
            ...
            for k in keep_keys:
                ...
                if pc_id in DEBUG_PROMPT_IDS:
                    logger.debug("[DBG %s-%s] %s", pc_id, k, item[k][:80])
    random.shuffle(examples)
    return examples

# Latent Adversarial Paraphrasing (LAP) Trainer
class LAPTtrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.lap_enabled:
            logger.info("LAP Trainer initialized with parameters: %s", json.dumps(self.lap_kwargs, indent=2))
    ...
    def _forward_with_perturbation(self, model: torch.nn.Module, inputs: Dict[str, Any], delta: torch.Tensor, layer_idx: int) -> Dict[str, torch.Tensor]:
        """
        Performs a forward pass with a perturbation `delta` injected at `layer_idx`
        This implementation is specific to Gemma-like models
        """
        base_model = model.model if isinstance(model, PeftModel) else model
        
        input_ids = inputs["input_ids"]
        attention_mask = inputs.get("attention_mask")
        
        # This logic is adapted from `transformers.models.gemma.modeling_gemma.GemmaModel.forward`
        if attention_mask is not None and hasattr(base_model, "_prepare_4d_causal_attention_mask"):
            attention_mask = base_model._prepare_4d_causal_attention_mask(
                attention_mask, (input_ids.shape[0], input_ids.shape[1]), 0
            )

        hidden_states = base_model.embed_tokens(input_ids)

        for i in range(layer_idx):
            hidden_states, _, _ = base_model.layers[i](
                hidden_states, attention_mask=attention_mask, position_ids=None, use_cache=False
            )
        
        hidden_states = hidden_states + delta

        for i in range(layer_idx, len(base_model.layers)):
            hidden_states, _, _ = base_model.layers[i](
                hidden_states, attention_mask=attention_mask, position_ids=None, use_cache=False
            )
    ...
    def training_step(self, model: torch.nn.Module, inputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        Main training step override for LAP
        """
        # If LAP is not enabled or we randomly skip it (p_sample), do standard SFT
        use_lap_for_batch = self.lap_enabled and (random.random() < self.lap_kwargs['lap_p_sample'])
        if not use_lap_for_batch:
            return super().training_step(model, inputs)

        # LAP Path
        model.eval() # Use eval mode for inner loop to get stable losses without dropout

        # Fetch LAP parameters
        ...
        try:
            # Get Original Hidden State and Loss (J0)
            with torch.no_grad():
            ...
            # Use log_lambda for stability as per paper
            ...
            for i in range(t_inner):
                ...
                # Forward pass with current perturbation to get J_delta
                ...
                # Lagrangian for delta update (minimize this)
                ...
                # Backpropagate to get gradients for delta
                (-lagrangian_for_delta).backward() # Maximize by minimizing the negative
                delta_optimizer.step()

                # Update lambda (no gradients needed)
                with torch.no_grad():
                ...
                if self.state.global_step % self.args.logging_steps == 0 and i == t_inner - 1:
                    logger.info(
                        "LAP Inner (step %d, iter %d): J0=%.4f, J_delta=%.4f, ||d||=%.4f, lambda=%.4f",
            ...
            # Fallback to standard SFT on error
            return super().training_step(model, inputs)
            
        # Outer Loop: Update Model Parameters
        model.train() # Switch back to train mode

        # Compute final loss with the adversarial perturbation for model update
        outputs_final = self._forward_with_perturbation(model, inputs, final_delta, layer_idx)
        loss = outputs_final['loss']
        
        # The Trainer's main logic will handle gradient accumulation and optimizer step
...
def main(argv=None):
    ...
    if full_ft:
        ...
        lcfg = LoraConfig(
            r=args.lora_rank, lora_alpha=args.lora_alpha, target_modules=mods,
    ...
    targs = TrainingArguments(
        ...
        bf16=args.bf16, fp16=not (args.bf16 or args.bnb_8bit_optim),
        ...
        group_by_length=True, learning_rate=args.learning_rate, weight_decay=args.weight_decay,
        lr_scheduler_type=args.lr_scheduler_type, warmup_ratio=args.warmup_ratio, max_grad_norm=0.3,
        logging_steps=args.logging_steps, logging_first_step=True,
        report_to=['wandb'], deepspeed=ds_cfg, seed=args.seed,
    )
    ...
    class StepDigest(TrainerCallback):
        def on_log(self, args, state, control, logs=None, **kwargs):
            ...
            if "eval_loss" in logs:
                logger.info("step %4d | eval_loss  %.4f | perplexity %.2f", state.global_step, logs["eval_loss"], math.exp(logs["eval_loss"]))

    trainer_class = LAPTtrainer if args.use_lap else Trainer
    lap_kwargs = {
        "use_lap": args.use_lap, "lap_layer": args.lap_layer, "lap_t_inner": args.lap_t_inner,
        "lap_p_sample": args.lap_p_sample, "lap_epsilon": args.lap_epsilon,
        "lap_delta_lr": args.lap_delta_lr, "lap_lambda_lr": args.lap_lambda_lr
    }

    trainer = trainer_class(
        model=model, args=targs, train_dataset=train_ds, eval_dataset=val_ds,
        data_collator=collator, callbacks=[StepDigest(), EarlyStoppingCallback(early_stopping_patience=args.early_stopping_patience)],
    ...
    val_loader = torch.utils.data.DataLoader(
        val_ds.remove_columns(["style"]), batch_size=args.batch_size * 2, shuffle=False, collate_fn=collator
    )
    styles = val_ds["style"]
    model.eval(); losses_by_style = defaultdict(list)
    with torch.no_grad():
        for idx, batch in enumerate(val_loader):
            # This needs to be device-aware
            batch = {k: v.to(model.device) for k, v in batch.items()}
            out = model(**batch)
            bsz = batch["input_ids"].size(0)
            sl = styles[idx * bsz : (idx + 1) * bsz]
            # Ensure loss is a single item
            loss_item = out.loss.item()
            for s in sl:
                losses_by_style[s].append(loss_item)
    ...
    for sty, lv in sorted(style_avg.items(), key=lambda x: x[1], reverse=True):
        logger.info("VAL-loss %-25s %.4f", sty, lv)
    ...
    if UNEXPECTED_EVENTS:
        ...
        logger.info("No unexpected events were logged.")
