#!/usr/bin/env python
"""
OLDER VERSION!!! this one does not support coninuing when checkpoint stopped
...
class LAPTtrainer(Trainer):
    """
    HF trainer that implements Latent Adversarial Paraphrasing (LAP)
    """
    ...
    def lap_training_step(self, model, inputs):
        ...
        try:
            ...
            # 3. Inner optimisation loop (min-max game)
            for i in range(t_inner):
                # We need to re-attach the hook in each loop iteration
                ...
                # L(δ, λ) = -||δ||₂ + λ * (|J_δ - J₀| - ε)
                # We want to minimise L wrt δ, and maximise wrt λ
                ...
                # We do this via gradient ascent on L wrt λ
...
def make_arg_parser():
    ...
    p.add_argument("--lap_t_inner", type=int, default=3, help="Number of inner loopsoptimization steps for LAP.")
...
def main():
    ...
    if is_full_ft:
        logging.info("Configuring for FULL-PARAMETER fine-tuning.")
        ...
        logging.info("Configuring for LoRA fine-tuning.")
