...
def content_score_ok(
    score: int,
    require_min: int,
    precise: Optional[int] = None,
) -> bool:
    """If precise is set, require exact match. Otherwise require >= require_min (current default behavior)."""
    if precise is not None:
        return score == precise
    return score >= require_min
...
def analyze_first_style_counts_firstfile(
    ...
    precise_content_score: Optional[int],
) -> Dict[str, int]:
    from collections import Counter
    ctr = Counter()
    for item in data:
        for ph in item.get("paraphrases", []):
            sc = int(ph.get("paraphrase_content_score", 0))
            if not content_score_ok(sc, require_content_score, precise_content_score):
                continue
...
def load_first_rawsplit(
    ...
    style_counts = analyze_first_style_counts_firstfile(
        data, allowed_styles, require_content_score, precise_content_score
    )
    ...
    _cmp = "==" if precise_content_score is not None else ">="
    _thr = precise_content_score if precise_content_score is not None else require_content_score
    logging.info(
        "FIRST: %d styles pass content_score %s %d and count>=%d",
        len(valid_styles), _cmp, _thr, first_min_style_count
    )
...
def load_examples_rawsplit(
    ...
    if is_first_format(data):
        return load_first_rawsplit(
            ...
            precise_content_score=precise_content_score,
...
def load_examples_from_tokenized(
    ...
    if is_first_format(raw):
        ...
        style_counts = analyze_first_style_counts_firstfile(
            raw, style_whitelist, require_content_score, precise_content_score
        )
        valid_styles = {s for s, c in style_counts.items() if c >= first_min_style_count}
        ...
        for item in raw:
            ...
            for ph in item.get("paraphrases", []):
                sc = int(ph.get("paraphrase_content_score", 0))
                # Preserve original equality-to-require_content_score behavior unless precise is set.
                if precise_content_score is not None:
                    if sc != precise_content_score:
                        continue
                else:
                    if sc != require_content_score:
                        continue
                sty = ph.get("instruct_type")
                if not sty or (valid_styles and sty not in valid_styles):
                    continue
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--precise-content-score", dest="precise_content_score", type=int, default=None,
                   help="FIRST dataset: if set, require paraphrase_content_score to equal this value exactly; otherwise ignored.")
...
def main():
    ...
    if args.use_pretokenized:
        ...
        flat_queue, results_map = load_examples_from_tokenized(
            ...
            precise_content_score=args.precise_content_score,
