...
def make_arg_parser():
    ...
    p.add_argument(
        ...
        action="store_true",
        help="After each checkpoint save, delete optimizer/scheduler files to keep checkpoints small.",
    )   
    p.add_argument(
...
#    model.config.attention_dropout = 0.05
    ...
    try:
        ...
        logging.info("Answer length p95 (tokens): %d — use this as max_new_tokens for inference.", p95_len)
    ...
    class ModelOnlyCheckpointCallback(TrainerCallback):
        """Delete heavy optimizer/scheduler files right after each save."""
        def on_save(self, args, state, control, **kwargs):
            try:
                ckpt_dir = Path(args.output_dir) / f"checkpoint-{state.global_step}"
                # Files that make checkpoints huge:
                for fname in ("optimizer.pt", "optimizer.bin", "scheduler.pt"):
                    fpath = ckpt_dir / fname
                    if fpath.exists():
                        fpath.unlink()
                # Leave trainer_state.json and rng_state.pth in place for resuming
                logging.info("Pruned optimizer/scheduler from %s", ckpt_dir)
            except Exception as e:
