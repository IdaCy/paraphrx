...
def make_arg_parser() -> argparse.ArgumentParser:
    ...
    #               help='Enable DeepSpeed ZeRO-2 CPU offload (fallbacks if unavailable)')
    p.add_argument('--use_deepspeed', action='store_true',
                   help='Enable DeepSpeed ZeRO-2 CPU offload (fallbacks if unavailable)')
    p.add_argument('--deepspeed_config', type=str, default='ds_zero2.json',
                   help='Path to DeepSpeed ZeRO-2 offload config JSON')
    p.add_argument('--bnb_8bit_optim', action='store_true',
                   help='Use 8-bit Adam optimizer (bitsandbytes) instead of fp16/fp32')
...
def main(argv: List[str] | None = None) -> None:
    ...
    # — add DeepSpeed / 8-bit Adam / step-eval / group-by-length
    ds_config = args.deepspeed_config if getattr(args, 'use_deepspeed', False) else None
    # verify DeepSpeed is importable
    if args.use_deepspeed:
        try:
            importlib.import_module('deepspeed')
        except ImportError:
            logging.warning("DeepSpeed not installed—continuing without ZeRO offload")
            ds_config = None
    ...
    train_args = TrainingArguments(
        ...
        # do a single eval every 5k steps instead of every epoch
        eval_strategy="steps",
        eval_steps=5000,
        save_strategy="steps",
