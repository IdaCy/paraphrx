"""
python3 f_finetune/src/compare_script02.py \
...
def load_model_and_tokenizer(model_path: str, base_model_path: str, device: str, attn_impl: Optional[str] = None, dtype_str: str = "bfloat16") -> tuple[PreTrainedModel, AutoTokenizer, str]:
    ...
    if (path / "adapter_config.json").exists() and PeftModel is not None:
        ...
        model = AutoModelForCausalLM.from_pretrained(base_model_path, torch_dtype=torch_dtype, device_map={"": device})
...
def main():
    ...
    base_model, tokenizer, base_model_id = load_model_and_tokenizer(args.base_model_path, args.base_model_path, device, attn_impl, dtype_str=args.dtype)
    ft_model, _tok2, ft_model_id = load_model_and_tokenizer(args.ft_model_path, args.base_model_path, device, attn_impl, dtype_str=args.dtype)
