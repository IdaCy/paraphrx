...
def flatten_examples(examples: List[dict]) -> Tuple[List[Tuple[str, str, str]], Dict[str, Dict[str, str]]]:
    ...
    for ex in examples:
        ...
        flat_queue.append((uid, key, prompt))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Labels for each JSON in --data_paths "
            "(e.g. alpaca gsm8k mmlu); must match length of --data_paths"
        ),
    )
    ...
    if args.datasets and len(args.datasets) != len(args.data_paths):
        parser.error("--datasets must have the same number of entries as --data_paths")
    # if no --datasets passed, default all to “alpaca”
    datasets = args.datasets or ["alpaca"] * len(args.data_paths)
    path_ds_pairs = list(zip(args.data_paths, datasets))
