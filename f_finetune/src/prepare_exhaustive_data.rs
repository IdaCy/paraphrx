/*
cargo prepare_exhaustive_data \
    --paraphrases-file a_data/alpaca/paraphrases_500.json \
    --answers-file     c_assess_inf/output/alpaca/gemma-2-2b-it/all.json \
    --scores-file      c_assess_inf/output/alpaca_answer_scores/gemma-2-2b-it.json \
    --out-file         f_finetune/data/exh_gemma.json
*/
...
struct Cli {
    ...
    #[arg(long)]
    scores_file: PathBuf,
    #[arg(long = "out-file", value_name = "PATH", default_value = "f_finetune/data/alpaca_gemma-2-2b-it.json")]
    out_file: PathBuf,
...
struct ParaphraseEntry {
    ...
    paraphrase: String,
    answer: String,
    // Task fulfilment score (first element of the scores array)
    task_score: i64,
    // **New field**: full array of scores coming from the scores-file
    scores: Vec<i64>,
...
fn main() -> Result<()> {
    ...
    for prompt in paraphrase_rows {
        ...
        // gather paraphrases
        // (ptype, paraphrase, answer, task_score, full_scores)
        let mut items: Vec<(String, String, String, i64, Vec<i64>)> = Vec::new();
        for (key, val) in prompt.as_object().unwrap() {
            ...
            // Extract full vector of scores (may be empty if missing)
            let scores_vec: Vec<i64> = scores_obj
                .get(key)
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_i64()).collect())
                .unwrap_or_default();
            let task_score = scores_vec.first().copied().unwrap_or(0);

            items.push((key.clone(), paraphrase, answer, task_score, scores_vec));
        }
        if items.is_empty() {
            continue;
        }
        ...
        // rank & bucket
        items.sort_by(|a, b| b.3.cmp(&a.3)); // sort by task_score desc

        // group contiguous identical task_scores
        ...
        for (rank_idx, (_score, vec)) in score_groups.into_iter().enumerate() {
            for (ptype, text, answer, task_score, all_scores) in &vec {
