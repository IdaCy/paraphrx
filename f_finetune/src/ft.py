...
normal
python ft.py \
  --data_paths path_or_artifact.json \
  --model_path f_finetune/model \
  --output_dir out/exp1 \
  --run_name gemma_paraphrx

pre-tokenised:
python ft.py \
  --use_pretokenized \
  --tokenized_data_path /path/to/tokenized_dataset \
  --data_paths dummy.json \
  --model_path f_finetune/model \
  --output_dir out/exp_pretok \
  --run_name gemma_pretok

LAP training:
python ft.py \
  --data_paths path_or_artifact.json \
  --model_path f_finetune/model \
  --output_dir out/exp_lap \
  --run_name gemma_lap \
  --use_lap \
  --lap_layer 12 --lap_t_inner 3 --lap_p_sample 0.5 \
  --lap_epsilon 0.05 --lap_delta_lr 1e-2 --lap_lambda_lr 1e-3
"""
...
def make_arg_parser():
    ...
    # LAP toggles (kept off by default to preserve Script 1 path)
    p.add_argument("--use_lap", action="store_true",
                   help="Enable LAP training step (structure identical otherwise).")
...
def main(argv=None):
    ...
    # Resolve dataset files / Artifacts (Script 1 default path)
    dataset_paths = []
    hf_dataset_dir = None  # NEW: detect HF dataset directories passed via --data_paths

    for spec in args.data_paths:
        p = Path(spec)

        # NEW: if a directory with dataset_dict.json -> treat as load_from_disk dataset
        ...
        if p.exists():
            ...
            if not files:
                raise FileNotFoundError(f"No JSON in artifact %s (downloaded into %s)" % (spec, str(ddir)))
            dataset_paths.append(str(files[0]))

    # NEW: auto-enable pretokenized mode if an HF dataset dir was passed via --data_paths
    ...
    def _get_eot_id(tok):
        ...
        # Fallback safely to eos if template/token not present
    ...
    # Make generation stop on EOS or end-of-turn by default (useful for any later generate())
    ...
    if not using_pretok:
        # Load examples (Script 1 default behaviour)
        if args.answers_path:
            if len(dataset_paths) != 1:
                raise ValueError(
                    "When --answers_path is provided, please pass exactly one prompts JSON in --data_paths."
                )
        ...
        try:
            ...
            logging.info(
                "Answer length p95 (tokens): %d — use this as max_new_tokens for inference.", p95_len
            )
        ...
        # Alternate path: use pre-tokenized dataset (Script 2 capability)
    ...
    targs = TrainingArguments(
        ...
        save_total_limit=args.save_total_limit,  # keep >=2 to avoid deleting best
    ...
    # Trainer class: vanilla (default) or LAP-augmented if requested
    trainer_cls = LAPTtrainer if args.use_lap else Trainer
    trainer_kwargs = dict(
        model=model,
        args=targs,
    ...
    if args.resume_checkpoint:
        ...
        # Fix dangling best_model_checkpoint to avoid "File not found" on resume
        try:
        ...
        if (not opt_file.exists()) and (not sched_file.exists()):
            # Model-only checkpoint detected -> catch up LR scheduler
            try:
    ...
    with torch.no_grad():
        ...
        for idx, batch in enumerate(val_loader):
            # Create a dictionary with only the keys the model accepts
            model_inputs = {
                k: v.to(model.device) 
                for k, v in batch.items() 
                if k in ["input_ids", "attention_mask", "labels"]
            }
            # Pass only the filtered inputs to the model
